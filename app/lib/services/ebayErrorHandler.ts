...
export class EbayErrorHandler {
  ...
  static async handleEbayError(error: any, context: string = 'eBay API'): Promise<never> {
    await logToDebug('EBAY_ERROR', 'Handling eBay API error', {
      context,
    ...
    if (error.response?.data) {
      ...
      if (ebayResponse.errors && ebayResponse.errors.length > 0) {
        ...
        await logToDebug('EBAY_ERROR', 'eBay API returned structured error', {
          errorId: primaryError.errorId,
          domain: primaryError.domain,
          category: primaryError.category,
          message: primaryError.message,
          longMessage: primaryError.longMessage,
          hasMappedError: !!mappedError
        }, 'ERROR');
        ...
        if (mappedError) {
          ...
          throw await this.handleErrorByCategory(primaryError, error.response.status);
        }
      }
      ...
      if (ebayResponse.warnings && ebayResponse.warnings.length > 0) {
        ...
        await logToDebug('EBAY_ERROR', 'eBay API Warning', {
          context,
          warningId: warning.errorId,
    ...
    if (error.response?.status) {
      await logToDebug('EBAY_ERROR', 'Handling HTTP status error without eBay structure', {
        statusCode: error.response.status,
    ...
    if (error.code === 'ECONNREFUSED' || error.code === 'ENOTFOUND') {
      await logToDebug('EBAY_ERROR', 'Network/connection error', {
    ...
    await logToDebug('EBAY_ERROR', 'Generic error fallback', {
      errorMessage: error.message,
  ...
  private static async handleErrorByCategory(ebayError: EbayError, httpStatus: number): Promise<never> {
    await logToDebug('EBAY_ERROR', 'Handling error by category', {
